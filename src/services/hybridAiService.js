...
class HybridAiService {
  ...
  /**
   * Scrape website content using FireCrawl API with fallback
   */
  ...
  /**
   * Fallback scraping using simple HTTP request
   */
  ...
  generateIntelligentFallback(query, pageData) {
    ...
    // Determine query category and get appropriate response
  ...
  async processQuery(query, url, domContent = null) {
    ...
    try {
      ...
      // Step 2: Try AI services for response generation
      let aiResponse = null;
...
Provide a helpful, conversational response that guides the user to find what they need. Be specific about where to look and keep it friendly and concise.`;
      ...
      // Step 3: Use intelligent fallback if AI services failed
  ...
  calculateConfidence(query, pageData, aiResponse) {
    ...
    if (aiResponse.source === 'api') {
      confidence = 0.85; // Higher confidence for AI API responses
    }
    
    if (pageData && pageData.success) {
      confidence += 0.1; // Bonus for successful page scraping
    }
    
    // Check if query terms appear in page content
